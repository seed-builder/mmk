...
class CreateTableStStockChecks extends Migration
{
    ...
    public function up()
    {
	    ...
	    Schema::create('st_stock_check_items', function (Blueprint $table) {
		    ...
		    $table->decimal('finv_hqty')->default(0)->comment('期初库存余额箱数量（余额表取值）');
		    $table->decimal('finv_eqty')->default(0)->comment('期初库存余额瓶数量（余额表取值）');
		    $table->decimal('fcheck_hqty')->default(0)->comment('盘点箱数量');
		    $table->decimal('fcheck_eqty')->default(0)->comment('盘点瓶数量');
		    $table->decimal('fdiff_hqty')->default(0)->comment('盘点差异箱数量(库存减盘点)');
		    $table->decimal('fdiff_eqty')->default(0)->comment('盘点差异瓶数量(库存减盘点)');
