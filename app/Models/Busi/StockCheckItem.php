...
//            if($model->box_qty > 0 && $model->fmaterial_id > 0){
//                $material = Material::find($model->fmaterial_id);
//                $model->fcheck_hqty = $model->box_qty + round($model->bottle_qty / $material->fratio, 2);
//                $model->fcheck_eqty = $model->box_qty * $material->fratio + $model->bottle_qty;
//                //差值
//                $model->fdiff_hqty = $model->fcheck_hqty - $model->finv_hqty ;
//                $model->fdiff_eqty = $model->fcheck_eqty - $model->finv_eqty;
//            }
//        });
...
//		var_dump($attributes);
		if($this->id > 0 && !empty($this->material)){
		    ...
		    if(isset($attributes['fmaterial_id'])){
                $material = Material::find($attributes['fmaterial_id']);
            }
        }
        if(!empty($material) && !empty($attributes['box_qty']) && !empty($attributes['bottle_qty'])) {
