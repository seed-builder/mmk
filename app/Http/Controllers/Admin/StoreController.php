...
//		$affected = DB::update('update st_stores set fforbid_status = ? where id = ?', ['A', $id]);
	...
	public function updateNumber(){
        $stores = DB::select('select * from st_stores where fnumber=""');
        $n = 0;
        if(!empty($stores)){
            foreach ($stores as $store){
                ...
                if($postalcode){
                    ...
                    if($fn){
                        ...
                        $fnumber = $postalcode . sprintf('%05d', 1);
                    }
                    $affected = DB::update('update st_stores set fpostalcode=? , fnumber = ? where id = ?', [$postalcode, $fnumber, $store->id]);
                    $n ++;
                }
            }
        }
